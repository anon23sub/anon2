...
def dataset_load(appliances, train, test=None,n=19,split_factor=0.3):
    ...
    X = jnp.array(X).reshape(X.shape[0], n, 1)
    Y = jnp.array(Y)
    x_test,x_cal , y_test, y_cal = train_test_split(x_train, y_train, test_size=split_factor, random_state=42)
    return x_train, y_train,x_cal,y_cal, x_test, y_test, X_timestamp, scaler_x, scaler_y, X, Y